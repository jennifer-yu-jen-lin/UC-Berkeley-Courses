...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    if ((get_lat(city1)-lat)**2 + (get_lon(city1)-lon)**2) > ((get_lat(city2)-lat)**2 + (get_lon(city2)-lon)**2):
...
def sprout_leaves(t, values):
    ...
    """
    """
    values = [tree(x) for x in values if not is_tree(x)]
    branch = []
    for subtree in branches(t):
        if is_leaf(subtree):
            branch.append(tree(subtree, values))
            # return tree(subtree, values)
        else:
            new_subtree = tree(t, sprout_leaves(subtree, values))
            # branch.append(tree(t, sprout_leaves(subtree, values)))
            branch.append(new_subtree)
    # print_tree(tree(label(t), branch))
    return tree(label(t), branch)
    #return nut_finder(subtree)
    """
    if is_leaf(t):
