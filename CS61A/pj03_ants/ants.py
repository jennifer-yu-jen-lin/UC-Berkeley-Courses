...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        #"*** YOUR CODE HERE ***"
        # if not exit: # cannot use this! if exit==0, this statement will be True.
        if exit is not None:
            exit.entrance = self
        # END Problem 2
...
class Ant(Insect):
    ...
    def __init__(self, armor=1):
        ...
        self.doubledByQueenAlready = False
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self): # isinstance(place.ant, ContainerAnt) and 
                #print(1)
                ContainerAnt.contain_ant(place.ant, self)
            # If the current ant can contain the ant in the space, then it does.
            elif self.can_contain(place.ant): # isinstance(self, ContainerAnt) and 
                #print(2)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while check_place.entrance is not beehive:
            if check_num<=self.max_range and self.min_range<=check_num and check_place.bees:
        ...
        if self.place is not None:
            if random_or_none(self.place.bees):
                ...
                if self.armor <= amount:
                    for b in bee_list:
                        Insect.reduce_armor(b,self.damage+self.armor)
                else:
                    for b in bee_list:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting != 0: # digesting
            ...
            if random_or_none(self.place.bees):
                list_bee_to_be_eaten = list(self.place.bees)
                for bee_to_be_eaten in list_bee_to_be_eaten:
                    self.eat_bee(bee_to_be_eaten)
        # END Problem 6
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        #"*** YOUR CODE HERE ***"
        if random_or_none(self.place.bees):
            list_bee_to_be_attacted = list(self.place.bees)
            for bee_to_be_attacted in list_bee_to_be_attacted:
                Insect.reduce_armor(bee_to_be_attacted,self.damage)
        # END Problem 7
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #self.Place.add_insect(insect) ## cannot use this!!! 'Water' object has no attribute 'Place'
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.armor = armor
        self.checkFirstQueenAnt = QueenAnt.firstQueenAnt
        QueenAnt.firstQueenAnt = False
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        #"*** YOUR CODE HERE ***"
        if self.checkFirstQueenAnt==False:
            Insect.reduce_armor(self,self.armor)
        else:
            # Attacks the nearest bee
            ThrowerAnt.action(self, gamestate)
            # doubles the damage of all the ants behind her (that haven't already been doubled)
            check_place = self.place.exit
            while check_place is not None:
                if check_place.ant is not None:
                    if check_place.ant.doubledByQueenAlready==False:
                        check_place.ant.damage = check_place.ant.damage * 2
                        check_place.ant.doubledByQueenAlready = True
                    if isinstance(check_place.ant, ContainerAnt):
                        if check_place.ant.contained_ant is not None:
                            if check_place.ant.contained_ant.doubledByQueenAlready==False:
                                check_place.ant.contained_ant.damage = check_place.ant.contained_ant.damage * 2
                                check_place.ant.contained_ant.doubledByQueenAlready = True
    ...
    def reduce_armor(self, amount):
        ...
        #"*** YOUR CODE HERE ***"
        Insect.reduce_armor(self,amount)
        if self.checkFirstQueenAnt==True and self.armor==0:
            bees_win()
        # END Problem 13
    def remove_from(self, place):
        if self.checkFirstQueenAnt==False:
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    # "*** YOUR CODE HERE ***"
    def make_slow_method(gamestate):
        if gamestate.time%2!=1: # SlowThrower should cause slowness on odd turns
...
def make_scare(action, bee):
    ...
    # "*** YOUR CODE HERE ***"
    hasBeenScared = False
    def make_scare_method(gamestate):
        if hasBeenScared==False:
            bee.forwards = (not bee.forwards)
        action(gamestate)
    return make_scare_method
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    # "*** YOUR CODE HERE ***"
    original_bee_action = bee.action
    counter = 0
