...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda r: scheme_eval(r, env))
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return self.parent.lookup(symbol) # (WRONG, should use recursionBuiltinProcedure) self.parent.bindings[symbol]
        else:
        # END PROBLEM 2
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        # BEGIN PROBLEM 5
        # "*** YOUR CODE HERE ***"
        env.define(target, scheme_eval(expressions.rest.first,env)) # expressions.rest = (2) # expressions.rest = 2
        return target
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        binding_i = bindings.first
        validate_form(binding_i, 2, 2)
        names = Pair(binding_i.first, names)
        validate_formals(names)
        values = Pair(scheme_eval(binding_i.rest.first, env), values)
